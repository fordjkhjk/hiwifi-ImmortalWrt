...
files/etc/health_sample.sh              # 健康采样：每 5 分钟记负载/内存明细/CPU细分(iowait)/D状态进程数，双写 /tmp（内存盘）+ /root（闪存，重启不丢）；异常时另存详细现场到 /root/health-alert.log
...
```
2026-09-07 09:25:49 load=[0.22 0.36 0.30] avail=84236kB
                    mem=[free=36960 cached=92344 anon=51280 sunrecl=22044]
                    cpu=[u=7% s=10% io=0%]  Dproc=0
                    top=[31960:v2ray 26536:AdGuardHome 23604:mosdns]
```
...
| `top` | 占内存前三的进程 | v2ray / AdGuardHome / mosdns |
